...
def load_dog_policy(logdir, Cfg):
    ...
    ckpt = torch.load(logdir + '/checkpoints_dog/ac_weights_082000.pt', map_location=device)
...
def load_arm_policy(logdir, Cfg):
    ...
    device = torch.device("cpu")
    ckpt = torch.load(logdir + '/checkpoints_arm/ac_weights_082000.pt', map_location=device)
    actor_critic.load_state_dict(ckpt)
