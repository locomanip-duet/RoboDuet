...
# screen -dmS RoboDuet_Seed8765 bash -c "source ~/miniconda3/etc/profile.d/conda.sh && conda activate isaacgym && python scripts/auto_train.py --num_envs 4096 --seed 8765 --run_name RoboDuet --sim_device cuda:0 --resume; exec bash"
...
screen -dmS Cooperated_Seed8765 bash -c "source ~/miniconda3/etc/profile.d/conda.sh && conda activate isaacgym && python scripts/auto_train.py --wo_two_stage --num_envs 4096 --seed 8765 --run_name Cooperated --sim_device cuda:1; exec bash"
