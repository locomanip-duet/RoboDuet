...
class StateEstimatorVision:
    def __init__(self, lc, use_cameras=True):
        ...
        self.ee_pose_delta = np.zeros(6)
        self.ee_pose_delta[0] = 0.1
        ...
        self.ee_pose_delta_subscription = self.lc.subscribe("ee_pose_delta", self._ee_pose_cb)
    ...
    def get_command(self):
        ...
        return np.array([cmd_x, 0, cmd_yaw, *(self.ee_pose_delta[:6].tolist())])
    ...
    def _ee_pose_cb(self, channel, data):
        msg = april_message_lcmt.decode(data)
        self.ee_pose_delta = np.array(msg.ee_pose)
