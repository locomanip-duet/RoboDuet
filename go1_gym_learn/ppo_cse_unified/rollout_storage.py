...
class RolloutStorage:
    ...
    def add_transitions(self, transition: Transition):
        ...
        self.rewards[self.step].copy_(transition.rewards.view(-1, 2))
        self.dones[self.step].copy_(transition.dones.view(-1, 1))
        self.values[self.step].copy_(transition.values)
        self.actions_log_prob[self.step].copy_(transition.actions_log_prob.view(-1, 2))
    ...
    def compute_returns(self, last_values, gamma, lam):
        ...
        self.advantages = (self.advantages - self.advantages.mean(dim=0)) / (self.advantages.std(dim=0) + 1e-8)
