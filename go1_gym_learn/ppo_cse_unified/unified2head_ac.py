...
class Unified2ActorCritic(nn.Module):
    ...
    def action_mean(self):
        # return torch.cat([self.distribution_dog.mean, self.distribution_arm.mean], dim=-1)
        return self.distribution.mean

    @property
    def action_std(self):
        # return torch.cat([self.distribution_dog.stddev, self.distribution_arm.stddev], dim=-1)
        return self.distribution.stddev

    @property
    def entropy(self):
        # return self.distribution_dog.entropy().sum(dim=-1), self.distribution_arm.entropy().sum(dim=-1)
