...
class Runner:
    ...
    def __init__(self, env, device='cpu', run_name: str=None, resume=False, log_dir=None, debug=False):
        ...
        actor_critic = ActorCritic(
            num_obs = self.env.cfg.plan.dog_num_observations,
            num_privileged_obs = self.env.cfg.plan.dog_num_privileged_obs,
            num_obs_history =  self.env.cfg.plan.dog_num_obs_history,
            num_actions = self.env.cfg.plan.dog_actions).to(self.device)

        self.alg = PPO(actor_critic, device=self.device)
        self.num_steps_per_env = RunnerArgs.num_steps_per_env

        # init storage and model
        self.alg.init_storage(self.env.num_train_envs, self.num_steps_per_env, [self.env.cfg.plan.dog_num_observations],
                              [self.env.cfg.plan.dog_num_privileged_obs], [self.env.cfg.plan.dog_num_obs_history], [self.env.cfg.plan.dog_actions])
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False, eval_freq=100, curriculum_dump_freq=500, eval_expert=False, width=80, pad=35):
        from ml_logger import logger
        # initialize writer
        ...
        obs_dict = self.env.get_dog_observations()  # TODO: check, is this correct on the first step?
