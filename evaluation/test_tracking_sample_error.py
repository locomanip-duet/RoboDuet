...
class TrackingTester:
    def __init__(self, num_envs, ckpt_folder, sample_dir,
                 ckpt_number, headless, device, sim_device, net_type, mode, output_file):
        """
        ...
        self.output_dir = output_file.split('.')[0]
        self.sample_path = sorted(glob.glob(sample_dir + "/*.npy"))
        ...
        if net_type == "C":
            ...
            unified_ckpt_path = self.ckpt_folder + '/checkpoints_unified/ac_weights_' + self.ckpt_number + ".pt"
        ...
        os.makedirs(os.path.join("results", self.output_dir), exist_ok=True)
        
        # 创建文件处理器
        file_handler = logging.FileHandler(os.path.join("results", output_file))
    ...
    def load_unified_policy(self, ckpt_path, cfg):
        ...
        def policy(obs, info={}):
            ...
            hidden = body(torch.cat((obs_history[..., -Cfg.env.num_observations:], latent), dim=-1))
    ...
    def generate_command(self, item):
        data = np.load(item)
        lpyrpy = data
    ...
    def angle_between_vectors(self, v1, v2):
        # 确保向量已经归一化
        v1_normalized = F.normalize(v1, dim=-1)
        v2_normalized = F.normalize(v2, dim=-1)
        
        # 计算点积和叉积
        dot_product = (v1_normalized * v2_normalized).sum(dim=-1, keepdim=True)
        cross_product = torch.norm(torch.cross(v1_normalized, v2_normalized, dim=-1), dim=-1, keepdim=True)
    ...
    def compute_ee_pose_error(self, target_quat):
        ...
        abg = self.env.get_alpha_beta_gamma_in_base_coord(torch.arange(self.num_envs, device=self.device))
        lpy_error = torch.abs(lpy - self.env.commands_arm_obs[:, 0:3])
        abg_error = torch.abs(abg - self.env.commands_arm_obs[:, 3:6])
        ...
        angle_error = self.angle_between_vectors(ee_vec, target_ee_vector)
    ...
    def test(self, standing_flag):
        total_tracking_error_stack = []
        total_workspace_points = []
        total_count_survival = 0
        total_collision_num = 0
            
        num_steps = len(self.sample_path)
        for it, item in enumerate(self.sample_path):
            self.logger.info(f"testing time {it}")
            target_lpyrpy = self.generate_command(item)
            target_lpy = torch.from_numpy(target_lpyrpy[:, :3]).to(self.device)
            target_rpy = torch.from_numpy(target_lpyrpy[:, 3:]).to(self.device)
            ...
            if standing_flag == 1:
                ...
                target_vel = self.random_sample(self.vel_range_low, self.vel_range_high)
                assert 0, "Use vel"
            ...
            for run_time in range(self.num_eval_steps):
                ...
                if run_time > self.num_eval_steps * 2. / 3:
                    ...
                    # self.logger.info(f"lpy success: {(distance_error < self.lpy_threshold).sum() / distance_error.shape[0]}")
                    # self.logger.info(f"rpy success: {(geodesic_distance < self.rpy_threshold).sum() / geodesic_distance.shape[0]}")
            ...
            t2 = time.time()
            self.logger.info(f"use time: {t2 - t1}")
            self.logger.info(f"solvability: {(collision_mask.shape[0] - collision_mask.sum()) / collision_mask.shape[0]}")
            self.logger.info(f"survival rate: {(dead_flag.shape[0] - dead_flag.sum()) / dead_flag.shape[0]}")

            try:
                if workspace_points_flag.any():
                    workspace_points = self.lpy_to_local_xyz(self.env.commands_arm[workspace_points_flag, :3].view(-1, 3)).cpu().numpy()
                    total_workspace_points.append(workspace_points)
                    np.save(f"results/{self.output_dir}/reachable_points_{os.path.basename(item)}.npy", workspace_points)
                else:
                    self.logger.info(f"No valid workspace points for {os.path.basename(item)}")
            except Exception as e:
                self.logger.error(f"Error processing workspace points for {os.path.basename(item)}: {str(e)}")

                
            tracking_error_stack = torch.cat(tracking_error_stack, dim=0).reshape(-1, 12)
            errors = torch.mean(tracking_error_stack, dim=0)
            self.logger.info(f"Computed length      error: {errors[0]}")
            self.logger.info(f"Computed pitch       error: {errors[1]}")
            self.logger.info(f"Computed yaw         error: {errors[2]}")
            self.logger.info(f"Computed distance    error: {errors[6]}")
            self.logger.info(f"Computed alpha       error: {errors[3]}")
            self.logger.info(f"Computed beta        error: {errors[4]}")
            self.logger.info(f"Computed gamma       error: {errors[5]}")
            self.logger.info(f"Computed angle       error: {errors[7]}")
            self.logger.info(f"Computed geodesic    error: {errors[11]}")
            self.logger.info(f"Computed vel_x       error: {errors[8]}")
            self.logger.info(f"Computed vel_y       error: {errors[9]}")
            self.logger.info(f"Computed vel_yaw     error: {errors[10]}")

        total_num = num_steps * self.num_envs
        self.logger.info(f"[Total] solvability: {(total_num - total_collision_num) / total_num}")
        self.logger.info(f"[Total] survival rate: {(total_num - total_count_survival) / total_num}")
        ...
        self.logger.info(f"[Total] Computed length      error: {errors[0]}")
        self.logger.info(f"[Total] Computed pitch       error: {errors[1]}")
        self.logger.info(f"[Total] Computed yaw         error: {errors[2]}")
        self.logger.info(f"[Total] Computed distance    error: {errors[6]}")
        self.logger.info(f"[Total] Computed alpha       error: {errors[3]}")
        self.logger.info(f"[Total] Computed beta        error: {errors[4]}")
        self.logger.info(f"[Total] Computed gamma       error: {errors[5]}")
        self.logger.info(f"[Total] Computed angle       error: {errors[7]}")
        self.logger.info(f"[Total] Computed geodesic    error: {errors[11]}")
        self.logger.info(f"[Total] Computed vel_x       error: {errors[8]}")
        self.logger.info(f"[Total] Computed vel_y       error: {errors[9]}")
        self.logger.info(f"[Total] Computed vel_yaw     error: {errors[10]}")
       
        # import ipdb; ipdb.set_trace()
        # 在循环外部
        try:
            total_workspace_points = np.vstack(total_workspace_points)
            self.logger.info(f"Total workspace points shape: {total_workspace_points.shape}")
            
            if total_workspace_points.shape[0] > 3:  # 需要至少4个点来构建凸包
                hull = ConvexHull(total_workspace_points)
                volume = hull.volume
                self.logger.info(f"[Total] Available space volume is: {volume}")
            else:
                self.logger.warning("Not enough points to compute convex hull and volume")
            
        except Exception as e:
            self.logger.error(f"Error computing workspace volume: {str(e)}")
        
        if self.mode == "survival":
            self.logger.info(f"Survival rate is: {100 * (1 - total_count_survival / (num_steps * self.num_envs))} %")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--stand_still", action="store_true", help="")
    ...
    parser.add_argument("--mode", default="survival", help="", choices=['tracking', 'space', 'draw', 'survival'])
    parser.add_argument("--pc_path", default=None, help="")
    parser.add_argument("--net_type", default="C", help="", choices=['C', 'U'])
    parser.add_argument("--ckpt_folder", default=None, help="")
    parser.add_argument("--sample_dir", default=None, help="")
    parser.add_argument("--output_file", default="output.log", help="Name of the output log file")
    ...
    # args.ckpt_folder = "/home/pgp/agile/hybrid_improve_dwb/runs/ckpts/"+args.algo+"/"+args.seed
    # args.ckpt_folder = "/home/a4090/hybrid_improve_dwb/runs/download/go1_torque_0704"
    # args.ckpt_folder = "/home/a4090/hybrid_improve_dwb/runs/go1_arx_torque/2024-07-13/auto_train/232428.254725_seed7153"
    # args.ckpt_folder = "/home/a4090/hybrid_improve_dwb/runs/adapt_dofx10/2024-08-09/auto_train/155105.439947_seed9913"
    # args.ckpt_folder = "/home/a4090/hybrid_improve_dwb/runs/Cooperated/2024-09-26/auto_train/222815.236228_seed2423"
    ...
    runner = TrackingTester(
        ...
        output_file = args.output_file  # 新增这一行
    )
    
    if args.mode != "draw":
        runner.test(args.stand_still)
    else:
